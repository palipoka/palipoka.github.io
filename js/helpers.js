...
const getWeather = () => {
  ...
  if (cached) {
    ...
    if (dateDiffInMinutes(cached.fetchedAt, Date.now()) < 15) {
      render(
        `Температура ${cached.temp} °C в ${cached.name}, ${cached.state}. Ожидайте: ${cached.desc}.`
      );
      return;
    }
  }
  fetch(
    ...
    .then((geoData) => {
      ...
      fetch(
        `https://api.openweathermap.org/data/2.5/weather?units=metric&lang=ru&lat=${geoData.lat}&lon=${geoData.lon}&appid=${WEATHER_API_KEY}`
      )
        ...
        .then((weatherData) => {
          ...
          render(
            `Температура ${cacheData.temp} °C в ${cacheData.name}, ${cacheData.state}. Ожидайте: ${cacheData.desc}.`
          );
